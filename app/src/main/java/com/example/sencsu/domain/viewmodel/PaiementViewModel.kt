...
class PaiementViewModel @Inject constructor(
    ...
    fun initializeFormData(adherentId: String?, localAdherentId: Long?, montantTotal: Double?) {
    ...
    fun isReferenceValidForMode(reference: String, mode: String): Boolean {
    ...
    fun loadAdherent(adherentId: String?, localAdherentId: Long?) {
    ...
    fun addPaiement() {
        viewModelScope.launch {
            try {
                ...
                if (!isReferenceValidForMode(uiState.reference, uiState.modePaiement)) {
                    val message = if (uiState.modePaiement == "Wave" || uiState.modePaiement == "Orange Money") {
                        "La référence mobile money est invalide"
                    } else {
